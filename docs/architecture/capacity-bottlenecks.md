...
一个 Prefork Concurrency 4 的 Worker 同时消费文档解析、Embedding、批量生成和维护任务。大 PDF、OCR 或慢 Embedding 会占满四个进程，使小任务即使在不同队列也得不到执行槽。
...
- `maintenance` Job 因文档任务持续延迟。

**一期是否处理：必须做基础保护。**

一期保留一个 Worker Deployment，但必须设置队列优先级、单任务 Soft/Hard Time Limit、文件大小/页数限制、单团队并发和幂等 Job 租约。通过 Redis Semaphore 把 `ingestion + embedding` 并发限制为 3，为 `maintenance/llm_batch` 保留第 4 个 Worker 槽位。
