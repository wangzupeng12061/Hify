...
- `jobs_jobs` 由 `jobs` 模块拥有；业务模块只能通过 Jobs Contract 创建、查询和更新 Job 生命周期。
- `platform_outbox` 和 `platform_inbox_receipts` 由 `shared.infrastructure.messaging` 拥有，是模块表前缀规则的仅有例外。业务 Repository 禁止直接查询或更新它们。
...
| Platform Outbox/Inbox | Hot | 共享消息基础设施拥有，Partial Index、处理后 7 天删除 |
| Jobs | Medium/Hot | `jobs` 模块拥有，Pending Partial Index、90 天保留 |
...
- 业务状态和 Outbox 必须在同一本地事务提交，再异步执行外部副作用。
