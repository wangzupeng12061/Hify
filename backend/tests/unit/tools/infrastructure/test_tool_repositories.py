...
def test_tool_name_lookup_matches_case_insensitive_unique_index() -> None:
    session = SessionSpy()
    repository = SqlAlchemyToolDefinitionRepository(cast(AsyncSession, session))
    ...
    assert "lower(tools_definitions.name)" in sql
    assert "search" in params.values()
