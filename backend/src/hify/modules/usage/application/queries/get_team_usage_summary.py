...
class GetTeamUsageSummaryQuery:
    actor: ActorContext


class GetTeamUsageSummaryHandler:
    def __init__(self, unit_of_work_factory: UsageUnitOfWorkFactory) -> None:
        self._unit_of_work_factory = unit_of_work_factory

    async def handle(self, query: GetTeamUsageSummaryQuery) -> UsageSummaryInfo:
        require_read_usage(query.actor)
        async with self._unit_of_work_factory() as unit_of_work:
            input_tokens, output_tokens, total_tokens, cost_amount = (
                await unit_of_work.records.summarize_for_team(team_id=query.actor.team_id)
            )
